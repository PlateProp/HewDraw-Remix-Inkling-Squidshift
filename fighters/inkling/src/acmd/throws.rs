...
unsafe extern "C" fn game_catch(agent: &mut L2CAgentBase) {
    ...
    if StatusModule::situation_kind(agent.module_accessor) == *SITUATION_KIND_GROUND {
        ...
        sv_kinetic_energy!(set_speed, agent, FIGHTER_KINETIC_ENERGY_ID_STOP, speed_x * 1.0, 0.0);
        sv_kinetic_energy!(set_accel, agent, FIGHTER_KINETIC_ENERGY_ID_STOP, -0.8 * lr, 0.0);
    ...
    frame(lua_state, 7.0);
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 5.0, 0.0, 6.6, 0.0, Some(0.0), Some(6.6), Some(9.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
...
unsafe extern "C" fn game_catchdash(agent: &mut L2CAgentBase) {
    ...
    frame(lua_state, 9.0);
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 5.0, 0.0, 6.6, 4.0, Some(0.0), Some(6.6), Some(9.2), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
...
unsafe extern "C" fn game_catchturn(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        CATCH(agent, 0, Hash40::new("top"), 5.0, 0.0, 6.9, -4.0, Some(0.0), Some(6.9), Some(-14.0), *FIGHTER_STATUS_KIND_CAPTURE_PULLED, *COLLISION_SITUATION_MASK_GA);
    }
    game_CaptureCutCommon(agent);
    wait(lua_state, 2.0);
    if is_excute(agent) {
...
unsafe extern "C" fn game_throwf(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        if WorkModule::is_flag(boma, *FIGHTER_INKLING_INSTANCE_WORK_ID_FLAG_INK_SUCCESS){
            ...
            AttackModule::set_ink_value(boma, 0, 90.0);
...
unsafe extern "C" fn game_throwb(agent: &mut L2CAgentBase) {
    ...
    if is_excute(agent) {
        ...
        ATTACK_ABS(agent, *FIGHTER_ATTACK_ABSOLUTE_KIND_THROW, 0, 9.0, 45, 92, 0, 55, 0.0, 1.0, *ATTACK_LR_CHECK_F, 0.0, true, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_S, *COLLISION_SOUND_ATTR_NONE, *ATTACK_REGION_THROW);
