...
unsafe extern "C" fn effect_turnrun(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::EFFECT_FOLLOW(agent, Hash40::new("inkling_squid_change"), Hash40::new("trans"), 3, 0, 0, 0, 0, 0, 1, true);
    ...
    frame(agent.lua_state_agent, SQUID_FRAME+6.0);
    if macros::is_excute(agent) {
        macros::EFFECT_FLW_UNSYNC_VIS(agent, Hash40::new("inkling_squid_change"), Hash40::new("trans"), 0, 2.5, -2.5, 0, 0, 0, 1, true);
        macros::LAST_PARTICLE_SET_COLOR(agent,r,g,b);
        EffectModule::enable_sync_init_pos_last(agent.module_accessor);
    }
}
...
unsafe extern "C" fn sound_turnrun(agent: &mut L2CAgentBase) {
    ...
    frame(agent.lua_state_agent, SQUID_FRAME-5.0);
    if macros::is_excute(agent) {
